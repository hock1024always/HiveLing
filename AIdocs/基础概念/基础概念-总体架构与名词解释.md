...
- **MCP（Model Context Protocol）**
  - 大模型调用外部系统的统一协议。
  - 本项目中的 MCP 工具示例：
    - `student_profile_tool`：查学生/班级学情画像。
    - `course_material_tool`：查课程、章节教材内容及元数据。
    - `exam_bank_tool`：查题库题目。

- **SKILLS**
  - 基于 MCP 工具和 LLM 编排出的「高层工作流能力」。
  - 示例：
    - `lesson_plan_skill`：从教材 + 学情自动生成教案。
    - `ppt_generate_skill`：根据教案骨架生成 PPT 结构与内容。
    - `digital_teacher_skill`：生成数字人讲解脚本及视频。
...
```go
// 网关内部根据路由规则分发到本地模型或云端模型
type DefaultLLMGateway struct {
    localClient LocalModelClient
    cloudClient CloudModelClient
}

func (g *DefaultLLMGateway) Chat(ctx context.Context, req *ChatRequest) (*ChatResponse, error) {
    if shouldUseLocal(req.Model) {
        return g.localClient.Chat(ctx, req)
    }
    return g.cloudClient.Chat(ctx, req)
}
```
